...
int send_list() {
    ...
    addr_data.sin_addr.s_addr = inet_addr("192.168.0.101");
...
void menu() {
    while (1) {
        ...
        switch (choice) {
            ...
            case 7: {
                char cur_file[256], new_file[256];
                printf("Nhap ten file hien tai: ");
                fgets(cur_file, sizeof(cur_file), stdin);
                cur_file[strlen(cur_file) - 1] = 0;
                printf("Nhap ten file moi: ");
                fgets(new_file, sizeof(new_file), stdin);
                new_file[strlen(new_file) - 1] = 0;
                rename_file(cur_file, new_file);
                break;
            }
            case 8: {
                char filename[256];
